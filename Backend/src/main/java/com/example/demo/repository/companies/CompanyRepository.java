...
public interface CompanyRepository extends JpaRepository<Company, Integer> {
    ...
    @Query(value = "select distinct c from Company c left join fetch c.offices o where c.name = ?1 ")
    List<Company> getCompanies(String name);
    @Query(value = "select distinct c from Company c left join fetch c.offices o where c.name = ?1 or o = ?1")
    ...
    @Query(value = "select distinct c from Company c left join fetch c.companyOwner o where o.email = ?1")
