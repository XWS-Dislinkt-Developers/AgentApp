...
export class CompanyService {
 ...
 acceptRequest(id: any) {
  return this._http.post('http://localhost:8090/companyRequest/approveRequest', id, {'headers': this.headers()})
  }

  createCompanyRequest(request: ICompanyRequest) {
    return this._http.post('http://localhost:8090/companyRequest', request, {'headers': this.headers()})
    }

  getAllCompanies(){
    return this._http.get<Observable<ICompany>>('http://localhost:8090/company/getAll').pipe()
  }

  showCompany(id: number){
    return this._http.get<any>('http://localhost:8090/company/'+ id).pipe()
  }
  ...
  searchCompany(name: string){
    ...
    return this._http.post<any>('http://localhost:8090/company/searchName', name, {headers: loginHeaders}).pipe()
  }
